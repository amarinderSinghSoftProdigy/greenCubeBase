...
public class DownloadInfo {
    ...
    public static class Reader {
        ...
        public void updateFromDatabase(DownloadInfo info) {
            ...
            info.mvId = getInt(Downloads.COLUMN_MOVIEID);
...
//            info.mTypeDown = getInt(Downloads.COLUMN_TYPEDOWNLOAD);
            synchronized (this) {
...
//        Log.i("DownloadManager", "sendBroadcast");
...
//    }
    public int checkCanUseNetwork() {
        NetworkInfo networkType = mSystemFacade.getActiveNetworkType();
        if (networkType == null) {
            return NETWORK_NO_CONNECTION;
        }
...
//        return checkIsNetworkTypeAllowed(networkType);
    ...
    void startIfReady(long now) {
        if (Constants.countThreadDownload < Constants.MAX_DOWNLOADS_SAME_TIME) {
//            if (checkCanUseNetwork() != NETWORK_OK && !sendNotification) {
//                return;
//            }
            ...
            Log.d(Constants.TAG, "mStatus: "+mStatus+", isDownloading: "+DownloadThread.isDownloading(this));
            if (mStatus != Downloads.STATUS_SUCCESS && !DownloadThread.isDownloading(this)) {
                Constants.countThreadDownload += 1;
                sendNotification = true;
                Log.v(Constants.TAG, "countThreadDownload " + Constants.countThreadDownload);
            ...
            if (mStatus == Downloads.STATUS_PENDING && sendNotification) {
...
//        Log.v(Constants.TAG, "TYPEDOWN : " + mTypeDown);
    }
